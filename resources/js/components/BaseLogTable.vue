      ...
      <template v-if="logViewerStore.logs && logViewerStore.logs.length > 0">
        ...
        >
          ...
          <template v-for="(column, colIndex) in logViewerStore.columns">
            <td :key="`${log.index}-column-${colIndex}`" v-if="column.data_path === 'level'" class="truncate log-level">
              <span>{{ log.level_name }}</span>
            </td>

            <td :key="`${log.index}-column-${colIndex}`" v-else-if="column.data_path === 'datetime'" class="text-gray-900 whitespace-nowrap dark:text-gray-200">
          ...
          <td class="hidden text-xs text-gray-500 whitespace-nowrap dark:text-gray-300 dark:opacity-90 lg:table-cell">
            ...
            <tab-container v-if="logViewerStore.isOpen(index)" :tabs="getTabsForLog(log)">
              ...
              <tab-content tab-value="raw">
                ...
                <template v-if="hasContext(log)">
                  <p class="pt-2 mx-2 text-xs font-semibold text-gray-700 border-t lg:mx-8 dark:text-gray-400 lg:text-sm">Context:</p>
                  <pre class="log-stack" v-html="highlightSearchResult(prepareContextForOutput(log.context), searchStore.query)"></pre>
                </template>

                <div v-if="log.extra && log.extra.log_text_incomplete" class="px-8 py-4 italic text-gray-500">
